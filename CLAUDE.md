...
- `npm run atlas` — local-only infinite-canvas map of every archived sketch (port 5181; `?` shows shortcuts). Four lenses: `1` Clusters (thematic rooms), `2` Series (folders, chronological), `3` Timeline (months × cluster lanes), `4` Spectrum (measured color: hue wheel + achromatic column). Drag pans, ⌃-scroll/pinch zooms, `f` fits. The rail lists clusters, cross-series threads, and opportunity insights; selecting a node opens a detail panel with tags, nearby (cross-series similarity), and a copyable run command.
