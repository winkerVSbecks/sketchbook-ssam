...
export const sketch = ({ wrap, context, width, height }: SketchProps) => {
  ...
  for (let i = 0; i < config.domainCount; i++) {
    const x1 = Random.rangeFloor(gap, width - gap);
    const y1 = Random.rangeFloor(gap, height - gap);
    const x2 = Random.rangeFloor(x1, width - gap);
    const y2 = Random.rangeFloor(y1, height - gap);
  ...
  wrap.render = ({ width, height, playhead }: SketchProps) => {
    ...
    polygonClusters.forEach((polygons, i) => {
      const visibleLimit = config.animate
        ? Math.floor(t * counts[i])
        : counts[i];
...
function generatePolygons(
  ...
  const colors = generateColors(hStart, count).reverse();
