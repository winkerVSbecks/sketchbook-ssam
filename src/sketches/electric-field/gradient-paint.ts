...
function drawDirectedPath(
  ...
  for (let i = 1; i < path.length; i++) {
    const t = path.length > 2 ? (i - 1) / (path.length - 2) : 0;
    ...
    for (let b = 0; b < bristles; b++) {
      const bf = bristles > 1 ? b / (bristles - 1) : 0.5; // 0..1
      const spread = (bf - 0.5) * w;
      const edgeDist = Math.abs(bf - 0.5) * 2; // 0 at centre, 1 at edges
      const bristleW = Math.max(0.3, (w / bristles) * (1 - edgeDist * 0.4));
      ctx.globalAlpha = 0.85 - edgeDist * 0.55;
      ctx.lineWidth = bristleW;
      ctx.strokeStyle = color;
      ctx.beginPath();
      ctx.moveTo(p1.re + nx * spread, p1.im + ny * spread);
      ctx.lineTo(p2.re + nx * spread, p2.im + ny * spread);
      ctx.stroke();
    }
  }
  ctx.globalAlpha = 1;
...
export const sketch = ({
  ...
  pane.addBinding(params, 'bristles', {
    label: 'Bristles',
    min: 1,
    max: 12,
    step: 1,
