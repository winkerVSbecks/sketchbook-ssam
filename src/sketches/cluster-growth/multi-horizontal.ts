...
function updateCluster(
  ...
  for (let y = 0; y < gridHeight; y++) {
    for (let x = 0; x < gridWidth; x++) {
      ...
      if (cluster.cells.length > gridWidth * gridHeight * 0.2) continue;
...
export const sketch = ({ wrap, context, width, height }: SketchProps) => {
  ...
  clusters.forEach((cluster, i) => {
    const x = Random.rangeFloor(0, gridWidth); // Math.floor((gridWidth * (i + 1)) / (clusters.length + 1));
    const y = Random.rangeFloor(0, gridHeight); // Math.floor(gridHeight / 2);
