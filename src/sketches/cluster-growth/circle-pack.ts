...
function createCluster(width: number, height: number, radius: number): Cluster {
  return {
...
export const sketch = ({ wrap, context, width, height }: SketchProps) => {
  ...
  const clusters = Array.from({ length: circles.length }, (_, idx) =>
    createCluster(gridWidth, gridHeight, circles[idx].r)
  );

  // Initialize each cluster at different positions
  clusters.forEach((cluster, idx) => {
    const circle = circles[idx];
    initCluster(cluster, gridWidth, gridHeight, circle.position);
  });
  ...
  wrap.render = ({ width, height, playhead }: SketchProps) => {
    ...
    //     }));
    //     cluster.cells = [];
    //     const x = Math.floor((gridWidth * (i + 1)) / (clusters.length + 1));
