...
export const sketch = async ({ wrap, context, width, height }: SketchProps) => {
  ...
  wrap.render = (props: SketchProps) => {
    ...
    // const w = Math.round(mapRange(playhead, 0, 0.5, 40, 80, true));
    // const h = Math.round(mapRange(playhead, 0.5, 1, 30, 80, true));
    const w = Math.round(mapRange(playhead, 0, 1, 40, 80, true));
    const h = Math.round(mapRange(playhead, 0, 1, 30, 80, true));
