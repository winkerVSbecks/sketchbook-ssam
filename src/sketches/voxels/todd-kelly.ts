...
function buildScene(playhead: number): Face[] {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      // Manhattan distance from nearest diamond center (tiled)
      const s = config.diamondSize;
      const cx = Math.round(c / s) * s;
      const cy = Math.round(r / s) * s;
      const dist = Math.abs(c - cx) + Math.abs(r - cy);

      const idx = ((dist + Math.floor(phase)) % numColors + numColors) % numColors;
