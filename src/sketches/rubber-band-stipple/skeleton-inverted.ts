...
export const sketch = ({
  ...
  // Figure/ground inversion: the palette color becomes the field, bg becomes
  // the void that gets carved by the hull and tendons.
  const voidColor = colors.pop()!;
  const strokeColor = `hsl(from ${colors.pop()!} h s l / 0.5)`;
  const fieldColor = colors.pop()!;
  ...
  wrap.render = ({ width, height, playhead }: SketchProps) => {
    ...
    // Field: solid palette color fills the canvas.
    ...
    // Hull filled with voidColor carves a discrete negative shape into the field.
    ...
    // Tendons drawn in voidColor extend the cut illusion across the void.
    context.strokeStyle = voidColor;
    ...
    // Stipple discs: clean gradient-filled circles — the only positive forms
    // inside the void. No rim, no dot, no shadow/highlight.
    for (const c of circles) {
      ...
      grad.addColorStop(1, shiftLightness(baseColor, -config.bevelStrength / 2));
