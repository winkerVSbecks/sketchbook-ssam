...
export const sketch = ({
  ...
  const shiftColor = (color: string, deltaL: number, deltaH: number): string => {
    const c = toHsl(color)!;
    return formatCss({
      ...c,
      h: ((c.h ?? 0) + deltaH) % 360,
      l: Math.min(1, Math.max(0, c.l + deltaL / 100)),
    });
  };
  ...
  // Ring count bucketed by stable maxR so the same node always gets the same
  // count regardless of its animated radius. Thresholds chosen so small discs
  // never look crowded (3 rings) and large discs show the full Albers study (5).
  const ringCountFor = (baseR: number): number => {
    if (baseR < 60) return 3;
    if (baseR < 110) return 4;
    return 5;
  };
  ...
  wrap.render = ({ width, height, playhead }: SketchProps) => {
    ...
    // inner ones clip the outer ones naturally via painter's algorithm. Ring
    // count is stable (depends only on maxR bucket, not animated r), and each
    ...
    for (let ci = 0; ci < circles.length; ci++) {
      ...
      // Use stable maxR (not animated r) for consistent ring count per node.
      const stableR = mapRange(
        noise(positions[ci].x, positions[ci].y, 0),
        -1, 1, config.minR, config.maxR, true,
      );
      const numRings = ringCountFor(stableR);

      for (let ring = 0; ring < numRings; ring++) {
        // Ring 0 = outermost (largest). Ring numRings-1 = innermost.
        const outerFraction = (numRings - ring) / numRings;
        const ringR = c.r * outerFraction;
        // Lightness brightens inward (like a lit center), hue drifts outward.
        const deltaL = (ring - (numRings - 1) / 2) * config.lightnessStep;
        const deltaH = (numRings - 1 - ring) * config.hueShift;
