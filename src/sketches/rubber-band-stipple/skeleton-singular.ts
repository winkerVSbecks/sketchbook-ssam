...
pane.addBinding(config, 'count', { min: 3, max: 10, step: 1 });
pane.addBinding(config, 'minR', { min: 20, max: 300, step: 1 });
pane.addBinding(config, 'maxR', { min: 60, max: 500, step: 1 });
...
export const sketch = ({
  ...
  // Pre-sample 3 stops along the palette at fixed positions so the gradient
  // identity stays stable across frames; only the angle rotates.
  const gradStop0 = colorMap(0.1);
  const gradStop1 = colorMap(0.5);
  const gradStop2 = colorMap(0.9);
  ...
  const ensurePositions = () => {
    ...
    // Wider margin with fewer, larger circles to prevent hull collapse at edges.
    const margin = config.maxR * 1.1 + driftAmt + 4;
    // Enforce larger minimum separation so circles spread across the frame.
    const minDist = ((width - 2 * margin) / Math.sqrt(config.count)) * 0.9;
    positions = [];
    let attempts = 0;
  ...
  wrap.render = ({ width, height, playhead }: SketchProps) => {
    ...
    // Gradient angle completes one full rotation per loop — gives the hull
    // a slow optical breathing without disturbing its silhouette.
    const gradAngle = playhead * Math.PI * 2;
    const cx = width / 2;
    const cy = height / 2;
    const reach = Math.hypot(width, height) * 0.5;
    const gx0 = cx + Math.cos(gradAngle) * reach;
    const gy0 = cy + Math.sin(gradAngle) * reach;
    const gx1 = cx - Math.cos(gradAngle) * reach;
    const gy1 = cy - Math.sin(gradAngle) * reach;
    const grad = context.createLinearGradient(gx0, gy0, gx1, gy1);
    grad.addColorStop(0, gradStop0);
    grad.addColorStop(0.5, gradStop1);
    grad.addColorStop(1, gradStop2);
    ...
    for (const c of circles) {
      ...
      const bevelGrad = context.createLinearGradient(
        c.x - c.r,
        c.y - c.r,
        c.x + c.r,
        c.y + c.r,
      );
      bevelGrad.addColorStop(0, shiftLightness(baseColor, config.bevelStrength));
      bevelGrad.addColorStop(1, shiftLightness(baseColor, -config.bevelStrength));
      context.fillStyle = bevelGrad;
      context.beginPath();
