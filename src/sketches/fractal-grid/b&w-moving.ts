...
import { loopNoise2D } from '../../loop-noise';
...
export const sketch = ({
  ...
  const clampOrigin = (o: Point): Point => [
    Math.max(margin[0], Math.min(width - margin[0], o[0])),
    Math.max(margin[1], Math.min(height - margin[1], o[1])),
  ];
  ...
  wrap.render = ({ playhead }: SketchProps) => {
    ...
    const origin: Point = clampOrigin([
      initialOrigin[0] + Math.round(width * loopNoise2D(playhead, 0.25)),
      initialOrigin[1] + Math.round(height * loopNoise2D(playhead, 0.25)),
    ]);
