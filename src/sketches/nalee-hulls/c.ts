...
const base = Random.pick(keys) as ColorType;

const bg = color(base, 2);
const border = color(base, 8);
const colors: [string[], string][] = /* Random.shuffle(keys) */ [base].map(
...
export const sketch = async ({ wrap, context, width, height }: SketchProps) => {
  ...
  const gapSystemCD = clipDomain(domain, hull);
  const gapSystem = createNaleeSystem(
    gapSystemCD,
    // config,
    { ...config, walkerCount: 1, pathStyle: 'solidStyle' },
    domainToWorld,
    [border],
    bg
  );
  ...
  wrap.render = (props: SketchProps) => {
    ...
    context.fillStyle = color(base, 3);
    ...
    systems.forEach((system, idx) => {
      const p = worldPolygons[idx];
      context.fillStyle = p.color;
      drawShape(context, p.polygon);
      // context.stroke();
      context.fill();
