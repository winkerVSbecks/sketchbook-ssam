...
pane.addBinding(config, 'sampleDetail', { min: 1, max: 3, step: 1 });
pane.addBinding(config, 'guides');
pane.addBinding(config, 'blobAmp', { min: 0, max: 0.8, step: 0.01 });
pane.addBinding(config, 'blobScale', { min: 0.5, max: 3, step: 0.05 });
pane.addBinding(config, 'blobFlow', { min: 0, max: 2, step: 0.05 });
...
// Radial noise displacement of a unit direction; time moves the sample point
// along a circle in the noise domain, so the deformation loops exactly once
// per playhead cycle.
const blobPoint = (n: THREE.Vector3, playhead: number): THREE.Vector3 => {
  const t = playhead * Math.PI * 2;
  const noise = Random.noise3D(
    n.x * config.blobScale + Math.cos(t) * config.blobFlow,
    n.y * config.blobScale,
    n.z * config.blobScale + Math.sin(t) * config.blobFlow,
  );
  return n.clone().multiplyScalar(1 + config.blobAmp * noise);
};

// Numerical surface normal of the deformed blob — used as the velocity fed
// through the tensor, so the forces respond to the deformation (a purely
// radial displacement would leave the normalized direction unchanged).
const blobNormal = (n: THREE.Vector3, playhead: number): THREE.Vector3 => {
  const [u, v] = perpendicularBasis(n);
  const eps = 0.01;
  const p = blobPoint(n, playhead);
  const pu = blobPoint(
    n.clone().addScaledVector(u, eps).normalize(),
    playhead,
  ).sub(p);
  const pv = blobPoint(
    n.clone().addScaledVector(v, eps).normalize(),
    playhead,
  ).sub(p);
...
// Unit directions where the blob is sampled for arrows and dots.
const sampleDirections = (detail: number): THREE.Vector3[] => {
  const geometry = new THREE.IcosahedronGeometry(1, detail);
  const points = surfacePoints(geometry);
  geometry.dispose();
  return points;
};

// Undeformed icosphere edges — endpoints are unit directions that get
// displaced through blobPoint each frame.
const wireEdges = (detail: number): Array<[THREE.Vector3, THREE.Vector3]> => {
...
interface FrameBases {
  samples: THREE.Vector3[];
  edges: Array<[THREE.Vector3, THREE.Vector3]>;
}

// One animation frame: the wireframe and the force arrows/dots, all drawn on
// the blob surface at this playhead. Rebuilt every frame — the caller resets
// the random seed first so the pencil jitter stays put while the blob moves.
const buildFrame = (
  F: number[][],
  ...
  const wireBuffer: StrokeBuffer = { positions: [], colors: [] };
  const wireTone = new THREE.Color(config.shapeTone);
  for (const [a, b] of bases.edges) {
    addSegments(
      wireBuffer,
      pencilLine(
        blobPoint(a, playhead),
        blobPoint(b, playhead),
        3,
        config.jitter * 0.6,
      ),
      wireTone,
  ...
  const wire = new THREE.LineSegments(
    strokeGeometry(wireBuffer),
    materials.stroke,
  );
  wire.visible = config.showShape;

  const deformed = bases.samples.map((n) => blobPoint(n, playhead));
  const normals = bases.samples.map((n) => blobNormal(n, playhead));
  const forces = normals.map((v) => forceAt(F, v));
...
const sketch: Sketch<'webgl2'> = ({
  ...
  let bases: FrameBases = {
    samples: sampleDirections(config.sampleDetail),
    edges: wireEdges(Math.max(0, config.resolution - 1)),
  };
  ...
  wrap.render = ({ playhead }) => {
    // Same seed every frame: identical jitter, so strokes deform smoothly
    // with the blob instead of boiling at 60fps.
    Random.setSeed(seed);
    if (field) {
      scene.remove(field);
      disposeField(field);
    }
    field = buildFrame(F, materials, bases, playhead);
    scene.add(field);
