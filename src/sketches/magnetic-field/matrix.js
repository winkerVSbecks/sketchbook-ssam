...
class Row {
  ...
  normalize() {
    ...
    for (
      let i = 0;
      i < this.size && Math.abs(this.get(i)) < Row.EPS();
      i += 1
    ) {
      this.clz = i + 1;
    }
...
class Matrix {
  ...
  static fromArray(arr, shape) {
    ...
    if (arrF.length < tot) {
      throw new TypeError(
        `Expected to have an array with length ${tot} but is has only ${arrF.length}`
      );
    }
  ...
  add(m, thisMatrix) {
    ...
    if (isMatrix && (m.rows != this.rows || m.cols != this.cols)) {
      throw new TypeError(
        `Dimensions does not match ${this.rows}x${this.cols} ${m.rows}x${m.cols}`
      );
    }
  ...
  setRow(row, r) {
    ...
    if (
      (Array.isArray(row) && row.length != this.cols) ||
  ...
  to_esc(thisMatrix) {
    ...
    while (true) {
      ...
      for (let i = 1; i < res.rows && ok; i += 1) {
        if (
          res.get(i).clz === res.get(i - 1).clz &&
          res.get(i).clz != res.cols
        ) {
          ok = false;
  ...
  GaussJordan() {
    if (this.rows > this.cols) {
      throw new TypeError(
        'Can not apply Gauss-Jordan Elimination to this matrix'
      );
    }
  ...
  dot(m, thisMatrix) {
    if (this.rows != m.rows || this.cols != m.cols) {
      throw new TypeError(
        'Dot product does not apply on different sizes matrices'
      );
    }
