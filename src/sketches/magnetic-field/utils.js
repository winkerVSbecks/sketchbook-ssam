...
class Plotter {
  ...
  drawGrid(sx, sy, col, str, highAxes) {
    ...
    let proj = (v1, v2) => {
      return Math.abs(v1.cross(v2)) / v2.abs();
    };
  ...
  fromMouse(x, y) {
    let trans = Matrix.fromArray([1, 0, 0, -1], [2, 2])
      .mul(this.zoomFactor)
      .inv();
    let cx = Matrix.fromArray([this.center.re, this.center.im], [2, 1]);
  ...
  drawPath(path, strokeC, strW, fillC, close, shapeFn) {
    ...
    let len = path.length;
    let cls =
      typeof close === 'undefined' || close === null ? close : close || CLOSE;
  ...
  drawSimpleArrow(cp1, cp2, col, nrm) {
    ...
    let sw = pA.sub(pB).abs() / 4;

    let _col = col || 255;

    this.drawSegment(cp1, newP2, _col, sw, 3);

    fill(_col);
    strokeWeight(2); //change size of thickness arrow
...
function svgToPath(dValue) {
  ...
  let pp = new (function (d) {
    ...
    this.getReflectedControlPoint = function () {
      ...
      return 'csqt'.indexOf(pc) === -1
        ? this.current
        : this.current.mul(2).sub(this.control);
    };
  ...
  while (!pp.isEnd()) {
    ...
    switch (pp.command.toLowerCase()) {
      ...
      case 'h': {
        while (!pp.isCommandOrEnd()) {
          var newP = new Complex(
            (pp.isRelativeCommand() ? pp.current.re : 0) + pp.getScalar(),
            pp.current.im
          );
      ...
      case 'v': {
        while (!pp.isCommandOrEnd()) {
          var newP = new Complex(
            pp.current.re,
            (pp.isRelativeCommand() ? pp.current.im : 0) + pp.getScalar()
          );
          pp.addMarker(newP, pp.current);
          pp.current = newP;
          ref = lineTo(pp.current);
      ...
      case 'a': {
        while (!pp.isCommandOrEnd()) {
          ...
          var currp = new Complex(
            (cos(xAxisRotation) * (curr.re - cp.re)) / 2.0 +
              (sin(xAxisRotation) * (curr.im - cp.im)) / 2.0,
            (-sin(xAxisRotation) * (curr.re - cp.re)) / 2.0 +
              (cos(xAxisRotation) * (curr.im - cp.im)) / 2.0
          );
          // adjust radii
          ...
          var s =
            ...
            sqrt(
              (pow(rx, 2) * pow(ry, 2) -
                pow(rx, 2) * pow(currp.im, 2) -
                pow(ry, 2) * pow(currp.re, 2)) /
          ...
          var centp = new Complex(
            (curr.re + cp.re) / 2.0 +
              cos(xAxisRotation) * cpp.re -
              sin(xAxisRotation) * cpp.im,
            (curr.im + cp.im) / 2.0 +
              sin(xAxisRotation) * cpp.re +
              cos(xAxisRotation) * cpp.im
          );
          // vector magnitude
          var m = function (v) {
          ...
          var a1 = a(
            [1, 0],
            [(currp.re - cpp.re) / rx, (currp.im - cpp.im) / ry]
          );
          // angle delta
