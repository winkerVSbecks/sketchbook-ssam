...
function randomTone(chroma = MAX_CHROMA) {
  return [Math.random(), Math.random() * 360, chroma || Math.random() * MAX_CHROMA];
}
...
type ColorHarmony = 'complementary' | 'triadic' | 'tetradic' | 'analogous' | 'split-complementary';

function generateHarmonicTones(baseHue: number, harmony: ColorHarmony = 'tetradic') {
  const tones: number[][] = [];
...
function generateVariations(hue: number, lightnesses: number[], chromas: number[]) {
...
const WHITE = [1, 0, 0];
const GRAY = [0.65, 0, 0];
const COLOR_TONES = Array.from({ length: 15 }, () => randomTone(MAX_CHROMA * 0.5)).sort(
  (a, b) => a[1] - b[1],
);
// const COLOR_TONES = [
//   BLACK, // black
...
export function getPalette({
  ...
  const tones = system == 0 ? generateHarmonicTones(baseHue, 'split-complementary') : BW_TONES;
