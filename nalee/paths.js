...
export function drawPath(context, walker, playhead, backgroundColor, paths) {
    ...
    paths.forEach((pts) => {
        pathStyles[walker.pathStyle](context, walker, pts, [l1, l2], t, backgroundColor);
    });
}
function solidStyle(context, walker, pts) {
...
function pipeStyle(context, walker, pts, [l1, l2], t, backgroundColor) {
    let l = 0;
...
function highlightStyle(context, walker, pts, [l1, _], t) {
    let l = 0;
    for (let i = 1; i < pts.length; i++) {
        const a = pts[i];
        const b = pts[i - 1];
    ...
    // middle
    const noiseHeadT = Random.noise3D(t, pts[0][0], pts[0][1]);
    const noiseTailT = Random.noise3D(t, pts[pts.length - 1][0], pts[pts.length - 1][1]);
...
function distressedStyle(context, walker, pts, [l1, _], t) {
    let l = 0;
    for (let i = 1; i < pts.length; i++) {
        const a = pts[i];
        const b = pts[i - 1];
...
function stitchStyle(context, walker, pts) {
    context.save();
    context.lineCap = 'round';
    context.lineJoin = 'round';
...
function thinLineStyle(context, walker, pts) {
    context.lineCap = 'round';
    context.lineJoin = 'round';
    // outer
    context.strokeStyle = walker.color;
...
function withNormalsStyle(context, walker, pts) {
    context.lineCap = 'round';
    context.lineJoin = 'round';
    // outer
    context.strokeStyle = walker.color;
...
function polkaLine(context, walker, pts) {
    context.save();
    context.lineCap = 'round';
    context.lineJoin = 'round';
...
function dimpleLine(context, walker, pts) {
    context.save();
    context.lineCap = 'round';
    context.lineJoin = 'round';
    const width = walker.size - walker.stepSize;
